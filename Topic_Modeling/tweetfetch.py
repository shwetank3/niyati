...
filetweet = open("mytweet.txt","w")   

for i in range(len(FIRST_TEN_TWEETS)):
    print(i, ': ', FIRST_TEN_TWEETS[i]['text'], '\n')
    try: 
     filetweet.writelines('%d\t%s\n' % (i,FIRST_TEN_TWEETS[i]['text']))
    except:
     filetweet.writelines('ERRRRRRRRRRRROOOOOOOOORRRRRRRRRRR\n')   
     print("\n\n\n\nERROR.... BAchao\n\n\n\n")   
     continue   
...
#print("%s\t%s\t%s\n",(len(dstore),len(FIRST_TEN_TWEETS),FIRST_TEN_TWEETS[i]))
